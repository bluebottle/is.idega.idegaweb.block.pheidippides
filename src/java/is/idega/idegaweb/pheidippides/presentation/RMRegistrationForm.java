...
public class RMRegistrationForm extends IWBaseComponent {
                ...
                case ACTION_CHARITY_SELECT :
                    if (getSession().getCurrentParticipant() != null) {
                        if (iwc.isParameterSet(PARAMETER_TEAM_NAME)) {
                            ...
                            String relayLeg = iwc
                                    .getParameter(PARAMETER_RELAY_LEG_FIRST);
                            holder.setLeg(relayLeg);

                            String teamName = iwc
                                    .getParameter(PARAMETER_TEAM_NAME);
                            Team team = new Team();
                            ...
                            String[] personalIDs = iwc.getParameterValues(
                                    PARAMETER_PERSONAL_ID_RELAY);
                            String[] datesOfBirth = iwc.getParameterValues(
                                    PARAMETER_DATE_OF_BIRTH);
                            String[] names = iwc
                                    .getParameterValues(PARAMETER_NAME);
                            String[] shirtSizes = iwc
                                    .getParameterValues(PARAMETER_SHIRT_SIZE);
                            String[] relayLegs = iwc
                                    .getParameterValues(PARAMETER_RELAY_LEG);
                            String[] emails = iwc
                                    .getParameterValues(PARAMETER_EMAIL);
                            ...
                            for (int i = 0; i < names.length; i++) {
                                if (names[i] != null && names[i].length() > 0) {
                                    ...
                                    if (getSession()
                                        ...
                                        participant
                                                .setPersonalId(personalIDs[i]);
                                    } else {
                                        try {
                                            participant.setDateOfBirth(format
                                                    .parse(datesOfBirth[i]));
                                        } catch (ParseException e) {
                                            e.printStackTrace();
                                        }
                                    }
                                    if (shirtSizes != null
                                            && shirtSizes[i] != null
                                            && shirtSizes[i].length() > 0) {
                                        participant.setShirtSize(getDao()
                                                .getShirtSize(Long.parseLong(
                                                        shirtSizes[i])));
                                    }
    ...
    private void showPersonSelect(IWContext iwc, PheidippidesBean bean) {
        ...
        facelet.setFaceletURI(
                iwb.getFaceletURI("registration/RM/personSelect.xhtml"));
        add(facelet);
    }
    ...
    private void showParticipant(IWContext iwc, PheidippidesBean bean) {
        ...
        FaceletComponent facelet = (FaceletComponent) iwc.getApplication()
                .createComponent(FaceletComponent.COMPONENT_TYPE);
        facelet.setFaceletURI(
                iwb.getFaceletURI("registration/RM/participant.xhtml"));
        add(facelet);
    }
    ...
    private void showRaceSelect(IWContext iwc, PheidippidesBean bean) {
        ...
        FaceletComponent facelet = (FaceletComponent) iwc.getApplication()
                .createComponent(FaceletComponent.COMPONENT_TYPE);
        facelet.setFaceletURI(
                iwb.getFaceletURI("registration/RM/raceSelect.xhtml"));
        add(facelet);
    }
    ...
    private void showRelayTeam(IWContext iwc, PheidippidesBean bean) {
        ...
        for (int i = 0; i < getSession().getCurrentParticipant().getRace()
                .getNumberOfRelayLegs() - 1; i++) {
            properties.add(new AdvancedProperty(String.valueOf(i + 2),
                    String.valueOf(i + 2)));
        }

        Map<String, Participant> partnerMap = new HashMap<String, Participant>();
        List<Participant> relayPartners = getSession().getCurrentParticipant()
                .getRelayPartners();
        ...
        bean.setRaceShirtSizes(getDao().getRaceShirtSizes(
                getSession().getCurrentParticipant().getRace()));

        FaceletComponent facelet = (FaceletComponent) iwc.getApplication()
                .createComponent(FaceletComponent.COMPONENT_TYPE);
        facelet.setFaceletURI(
                iwb.getFaceletURI("registration/RM/relayTeam.xhtml"));
        add(facelet);
    }

    private void showCharitySelect(IWContext iwc, PheidippidesBean bean) {
        String locale = iwc.getCurrentLocale()
                .equals(LocaleUtil.getIcelandicLocale()) ? "is" : "en";
        bean.setExternalCharities(getService().getExternalCharities(locale));

        FaceletComponent facelet = (FaceletComponent) iwc.getApplication()
                .createComponent(FaceletComponent.COMPONENT_TYPE);
        facelet.setFaceletURI(
                iwb.getFaceletURI("registration/RM/charitySelect.xhtml"));
        add(facelet);
    }

    private void showCharityTrinketSelect(IWContext iwc,
            PheidippidesBean bean) {
        FaceletComponent facelet = (FaceletComponent) iwc.getApplication()
                .createComponent(FaceletComponent.COMPONENT_TYPE);
        facelet.setFaceletURI(iwb
                .getFaceletURI("registration/RM/charityTrinketSelect.xhtml"));
        add(facelet);
    }
    ...
    private void showWaiver(IWContext iwc, PheidippidesBean bean) {
        FaceletComponent facelet = (FaceletComponent) iwc.getApplication()
                .createComponent(FaceletComponent.COMPONENT_TYPE);
        facelet.setFaceletURI(
                iwb.getFaceletURI("registration/RM/waiver.xhtml"));
        add(facelet);
    }
    ...
    private void showOverview(IWContext iwc, PheidippidesBean bean) {
        FaceletComponent facelet = (FaceletComponent) iwc.getApplication()
                .createComponent(FaceletComponent.COMPONENT_TYPE);
        facelet.setFaceletURI(
                iwb.getFaceletURI("registration/RM/overview.xhtml"));
        add(facelet);
    }
    ...
    private void showReceipt(IWContext iwc, PheidippidesBean bean) {
        FaceletComponent facelet = (FaceletComponent) iwc.getApplication()
                .createComponent(FaceletComponent.COMPONENT_TYPE);
        facelet.setFaceletURI(
                iwb.getFaceletURI("registration/RM/receipt.xhtml"));
        add(facelet);
    }
