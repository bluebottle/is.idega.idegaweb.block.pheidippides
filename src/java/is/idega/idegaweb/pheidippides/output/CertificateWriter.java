...
import is.idega.idegaweb.pheidippides.business.CertificatePrintingContext;
import is.idega.idegaweb.pheidippides.dao.PheidippidesDao;
import is.idega.idegaweb.pheidippides.data.Registration;

public class CertificateWriter extends DownloadWriter implements MediaWritable {
	...
	public void init(HttpServletRequest req, IWContext iwc) {
		try {
			...
			this.buffer = getDocumentBuffer(new CertificatePrintingContext(iwc, registration, locale));
			setAsDownload(iwc, iwrb.getLocalizedString("certificate_filename", "certificate") + ".pdf", this.buffer.length());
		}
		catch (Exception e) {
			e.printStackTrace();
		}
