...
public class PheidippidesWebService {
    ...
    public String registerRunner(
        ...
        WebServiceLoginSession loginSession = validateAndUpdateLoginSession(
                new is.idega.idegaweb.pheidippides.webservice.server.Session(
        ...
        ExternalCharity externalCharity = null;

        if (charityPersonalID != null) {
            List<ExternalCharity> charities = service
                    .getExternalCharities("is");
            for (ExternalCharity externalCharity2 : charities) {
                if (externalCharity2.getSsn().equals(charityPersonalID) || externalCharity2.getId().equals(charityPersonalID)) {
                    externalCharity = externalCharity2;
                    break;
                }
            }
        }
        ...
        Company company = dao
                .getCompanyByUserUUID(loginSession.getUser().getUniqueId());

        Race race = dao.getRace(company.getEvent(), d,
                IWTimestamp.RightNow().getYear(),
                (leg == null || "0".equals(leg)) ? false : true); // todo get
                                                                  // the correct
                                                                  // year
         ...
         * ShirtSizeGender.Male; } else if
         * (shirtSizeGender.equalsIgnoreCase("female")) { gender =
         * ShirtSizeGender.Female; } else if
         * (shirtSizeGender.equalsIgnoreCase("child")) { gender =
         * ShirtSizeGender.Child; }
         *
         * if (gender == null) { return "ERROR: Unknown gender"; }
         *
         * ShirtSizeSizes size = null; if (shirtSize.equalsIgnoreCase("XXXS")) {
         * size = ShirtSizeSizes.XXXS; } else if
         * (shirtSize.equalsIgnoreCase("XXS")) { size = ShirtSizeSizes.XXS; }
         * else if (shirtSize.equalsIgnoreCase("XS")) { size =
         * ShirtSizeSizes.XS; } else if (shirtSize.equalsIgnoreCase("S")) { size
         * = ShirtSizeSizes.S; } else if (shirtSize.equalsIgnoreCase("M")) {
         * size = ShirtSizeSizes.M; } else if (shirtSize.equalsIgnoreCase("L"))
         * { size = ShirtSizeSizes.L; } else if
         * (shirtSize.equalsIgnoreCase("XL")) { size = ShirtSizeSizes.XL; } else
         * if (shirtSize.equalsIgnoreCase("XXL")) { size = ShirtSizeSizes.XXL; }
         * else if (shirtSize.equalsIgnoreCase("XXXL")) { size =
         * ShirtSizeSizes.XXXL; }
         *
         * if (size == null) { return "ERROR: Unknown shirt size"; }
         *
         * ShirtSize shirtSizeEntry = dao.getShirtSize(size, gender); if
         * (shirtSizeEntry == null) { return
         * "ERROR: Could not find shirt size for this gender"; }
         *
         * RaceShirtSize rss = dao.getRaceShirtSize(race, shirtSizeEntry); if
         * (rss == null) { return
         * "ERROR: Shirt size not available for this distance"; }
         */
        ...
        holder.setExternalCharity(externalCharity);

        return service.storeWebserviceRegistration(holder, company,
                loginSession.getUser().getUniqueId(),
                LocaleUtil.getIcelandicLocale());
    }

    public is.idega.idegaweb.pheidippides.webservice.server.Charity[] getCharities(
            String locale) {
        List<Charity> charities = dao.getEvent(PheidippidesWebService.RM_ID)
                .getCharities();
        if (charities != null && !charities.isEmpty()) {
            is.idega.idegaweb.pheidippides.webservice.server.Charity[] ret = new is.idega.idegaweb.pheidippides.webservice.server.Charity[charities
                    .size()];
            int counter = 0;
            for (Charity charity : charities) {
                ...
                if (locale == null || !"en".equalsIgnoreCase(locale)) {
                    ...
                    ret[counter++]
                            .setDescription(charity.getEnglishDescription());
                }
        ...
        if (user == null) {
            try {
                LoginTable login = LoginDBHandler
                        .getUserLoginByUserName(identifier);
                if (login != null) {
                    user = login.getUser();
                }
            } catch (Exception e) {
    ...
    public CharityInformation getCharityInformation(String identifier) {
        ...
        List<Registration> regs = dao.getRegistrationForUser(event,
                IWTimestamp.RightNow().getYear(), user.getUniqueId());
        if (regs == null || regs.isEmpty()) {
            regs = dao.getRelayPartnerRegistrationForUser(event,
                    IWTimestamp.RightNow().getYear(), user.getUniqueId());
        }
        if (regs != null) {
            for (Registration registration : regs) {
                if (registration.getStatus().equals(RegistrationStatus.OK)
                        || registration.getStatus()
                                .equals(RegistrationStatus.RelayPartner)) {
                    ...
                    if (registration.getCharity() != null) {
                        ...
                        info.setCharityName(
                                registration.getCharity().getName());
                    } else {
                        info.setCharityID("");
                        info.setCharityName("");
                    }
                    ...
                    try {
                        country = getUserBusiness().getAddressBusiness()
                                .getCountryHome().findByPrimaryKey(new Integer(
            ...
            if (loginTable != null) {
                ...
                if (sessions != null && !sessions.isEmpty()) {
                    ...
                    while (it.hasNext()) {
                        ...
                        if (IWTimestamp.getMilliSecondsBetween(lastAccess,
                                now) <= getSessionTimeout()) {
                ...
                if (loginBean.verifyPassword(loginTable, password)) {
                    // check if corresponds to a company that is allowed to use
                    // the webservice
                    try {
                        Company company = dao.getCompanyByUserUUID(
                                loginTable.getUser().getUniqueId());
    ...
    private int getSessionTimeout() {
        String timeout = IWMainApplication.getDefaultIWApplicationContext()
    ...
    public boolean updateUserCharity(Session session, String personalID,
            String charityPersonalID)
                    throws RemoteException, SessionTimedOutException {
